...
* upgrade certificate
** edit ssl.crt/www.domain.tld.crt
** scp root@ns1.host.com:www.* .
** rm www.domain.tld.csr
** mv www.domain.tld.key ssl.key/
** dig domain.tld
** dig ns1.host.tld
** dig ns2.host.tld
** scp ssl.key/www.domain.tld.key ssl.crt/www.domain.tld.crt root@ns1.host.tld:/etc/perlbal/certs/
** ssh -l root ns1.host.tld
*** ubic restart perlbal
*** ubic status
** curl -I https://www.domain.tld
* server installation
** scp iptables.hardened root@$IP:/etc/sysconfig/iptables
*** restart iptables
** yum upgrade
** scp -R prep root@$IP:/root
*** sh prep/main.sh
** scp and install bind (lamp-named)
** cpanm Ubic Perlbal
** ubic-admin setup
*** scp scp perlbal.init root@$IP:/etc/init.d/perlbal
*** scp perlbal.service root@$IP:/etc/ubic/service/perlbal
** install nrpe
*** nagios/nrpe
